...
if [ "$(docker ps -q -f name=$CONTAINER_NAME)" ]; then
    ...
    print_color $CYAN "  - 前端: http://localhost:5173"
    print_color $CYAN "  - 前端 (區域網路): http://$(hostname -I | awk '{print $1}'):5173"
    ...
    print_color $YELLOW "  1. 檢查端口是否被占用: netstat -tlnp | grep -E ':(5173|3001)'"
